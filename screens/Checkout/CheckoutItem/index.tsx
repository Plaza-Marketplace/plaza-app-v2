...
import { createBuyNow, createPayment } from '@/services/stripe';
...
const ConfirmItemScreen = () => {
  ...
  const { data: product } = useGetProductById(parseInt(productId));
  ...
  const bottomButtonPress = async () => {
    if (currForm === FormType.PAYMENT) {
      ...
      const { paymentIntent, ephemeralKey, customer } = await createBuyNow(
        product?.id
      );
  ...
  if (!product || !userAddresses || !user) return <Loading />;

  const priceBeforeProcessing = product.price + product.shippingPrice;
  const price = product.price;
  const shipping = product.shippingPrice;
  const stripeProcessingFee = priceBeforeProcessing * 0.029 + 0.3;
        ...
        {currForm === FormType.PAYMENT && (
          <View style={styles.priceContainer}>
            ...
            <View style={styles.textRow}>
              ...
              <HeaderText>
                {formatPrice(priceBeforeProcessing + stripeProcessingFee)}
              </HeaderText>
            </View>
          </View>
        )}
...
export default ConfirmItemScreen;
